...
public class InventoryRepository : IInventoryRepository
{
    ...
    public InventoryEntity? AddToInventoryById(int legoSetId, int amountOfSets = 1, int? id = null)
    {
        using MySqlConnection connection = MySqlUtility.OpenConnection();
        using MySqlCommand? command = connection.CreateCommand();
        command.CommandText = "SELECT EXISTS(SELECT * FROM inventory_lego_set WHERE lego_set_id = @legoSetId AND inventory_id = @inventoryId)";
        if ((int) command.ExecuteScalar() == 1)
        {
            using MySqlCommand? updateCommand = connection.CreateCommand();lego_set
